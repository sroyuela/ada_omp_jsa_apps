...
package body Parallel.Functional_Reducing_Linked_List is

   protected body List is

      procedure Cancel is
      begin
         Outstanding_Reductions := 0;
      end Cancel;

--        procedure Dump (Header : String) is
--           Item : Node_Type := Elements (Elements'First);
--           Index : Node_Index := Node_Index'First;
--        begin
...
--
--           Dump_Loop : loop
--              Put_Line ("Index=" & Node_Index'Image (Index) &
--                        ", state=" & Element_State'Image (State (Index)) &
--                        ", Prev=" & Node_Index'Image (Item.Prev) &
--                        ", Next=" & Node_Index'Image (Item.Next));
--              exit Dump_Loop when Item.Next = Node_Index'First;
--              Index := Item.Next;
--              Item := Elements (Item.Next);
--           end loop Dump_Loop;
--           Put_Line ("Outstanding=" &
--                     Worker_Count_Type'Image (Outstanding_Reductions));
--        end Dump;
      ...
      begin
         ...
         Reduction_Node.Element :=
           Reducer (Item, Reduction_Node.Element);

         --  Now perform reduction of deleted nodes value,
         Reduction_Node.Prev.Element :=
           Reducer (Reduction_Node.Prev.Element,
                    Reduction_Node.Element);
...
--     end Dump;
   ...
   begin
      return Container.Value (Worker_Count_Type (Position));
   end Value;
